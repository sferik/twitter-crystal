...
client = Twitter::REST::Client.new(consumer_key, consumer_secret, token, secret)
```
...
**Post a new tweet/Delete a tweet**
```crystal
# This may be confusing, but in Twitter API docs, "update" means posting a tweet.
client.update("Good morning")

# In Twitter API docs, "status" means tweet.
